...
import { chapters as unsortedChapters } from "@/data/sangue-e-sucessao/chapters";
...
export default function SangueESucessaoPage() {
  ...
  return (
    <main className="min-h-screen bg-abyss text-stone-200 font-sans selection:bg-red-900/30 pb-20">
      {/* Navigation Bar */}
      <nav className="fixed w-full z-50 glass-panel border-b border-white/5 top-0 left-0 px-6 py-4 flex justify-between items-center backdrop-blur-md bg-abyss/80">
        <Link href="/" className="font-serif text-2xl text-red-500 font-bold tracking-tighter hover:text-white transition-colors">
          ACdC
        </Link>
      ...
      <div className="relative pt-32 pb-16 px-6 text-center overflow-hidden">
        <div className="absolute top-0 left-1/4 w-full h-full bg-red-900/10 blur-[120px] -z-10 pointer-events-none" />
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-900/10 border border-red-900/20 text-xs font-display tracking-widest text-red-400 mb-6">
          ...
          <p className="text-stone-400 max-w-2xl mx-auto text-lg leading-relaxed">
            A história de Enoch Thorne. Um mercenário que perdeu tudo em uma mesa de monstros e jurou afogar cada um deles no próprio sangue.
          </p>
        </motion.div>
      </div>
      ...
      <div className="container mx-auto px-4 max-w-3xl">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {/* Chapter Header */}
            <header className="mb-12 text-center border-b border-red-900/20 pb-8">
              <span className="block text-red-500 font-display tracking-[0.2em] text-sm mb-2 uppercase">
                ...
                <section key={index} className="relative">
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div className="ml-6 md:ml-0">
                      ...
                      {section.location && (
                        <div className="flex items-center gap-2 text-xs text-stone-500 mt-1 font-mono uppercase tracking-wide">
                          ...
                          <button
                            ...
                            className={`w-full text-left px-3 py-2.5 rounded-lg text-sm flex items-center justify-between transition-all group ${
                              index === currentChapterIndex
                                ? "bg-red-900/10 text-red-400 ring-1 ring-red-900/20"
                                : "text-stone-400 hover:bg-white/5 hover:text-white"
                            }`}
                          >
                            <span className="truncate pr-4">
                              <span className={`mr-2 font-mono text-xs ${index === currentChapterIndex ? "text-red-500/70" : "text-stone-600 group-hover:text-stone-400"}`}>
